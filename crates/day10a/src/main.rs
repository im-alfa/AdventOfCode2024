...
fn pathfinder(x: usize, y: usize, grid: &Grid, seen: &mut HashMap<(usize, usize), bool>) -> usize {
    let current_number = grid[y][x];

    if current_number == 48 + 9 {
        return match seen.insert((x, y), true).is_none() {
            true => 1,
            false => 0,
        };
    }
    ...
    for (nx, ny) in [
        ...
        if (0..ROWS).contains(&ny) && (0..COLS).contains(&nx) {
            ...
            if next_number == current_number + 1 {
                sum += pathfinder(nx, ny, grid, seen);
            }
        }
    }
...
fn main() {
    ...
    let ans = trailheads
        .iter()
        .map(|trailhead| pathfinder(trailhead.0, trailhead.1, &data, &mut HashMap::new()))
        .sum::<usize>();
